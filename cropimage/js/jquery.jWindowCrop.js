...
(function($){
	...
	$.jWindowCrop = function(image, options){
		...
		base.init = function(){
			...
			base.$frame.append('<div class="jwc_controls" style="display:'+(base.options.showControlsOnStart ? 'block' : 'none')+';"><span>Click to drag and reposition</span><a href="#" class="jwc_zoom_in"></a><a href="#" class="jwc_zoom_out"></a></div>');
			base.$frame.css({'position': 'relative', 'width': base.options.targetWidth, 'height': base.options.targetHeight}); //'overflow': 'hidden', 
			base.$image.css({'position': 'absolute', 'top': '0px', 'left': '0px'});
			initializeDimensions();

			base.$frame.find('.jwc_zoom_in').on('mousedown.'+base.namespace, function(){zoomInk=setInterval(base.zoomIn,50);return false;});
			base.$frame.find('.jwc_zoom_in').on('mouseup.'+base.namespace, function(){clearInterval(zoomInk);return false;});
			base.$frame.find('.jwc_zoom_in').on('click.'+base.namespace, base.zoomIn);
			base.$frame.find('.jwc_zoom_out').on('mousedown.'+base.namespace, function(){zoomOutk=setInterval(base.zoomOut,50);return false;});
			base.$frame.find('.jwc_zoom_out').on('mouseup.'+base.namespace, function(){clearInterval(zoomOutk);return false;});
