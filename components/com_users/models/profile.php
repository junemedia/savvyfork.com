...
class UsersModelProfile extends JModelForm
{
	...
	public function updateRecipe($userId,$recipeId,$recipeImage)
	{
		return $this->imageUpload($userId,$recipeId,$recipeImage,2097152,'');
	}
	
	private function imageUpload($userId,$recipeId,$file,$max,$msg){
        ...
        if(isset($file)){
				...
				$db = JFactory::getDBO ();
				$sql = "SELECT * FROM #__yoorecipe AS y WHERE y.id = ". (int)$recipeId;
				$db->setQuery($sql);
				$recipe = $db->loadObject();

				$dest = !empty($recipe)?JPATH_ROOT.DS.$recipe->picture:'';

				$result = false;
                //First check if the file has the right extension, we need jpg only
                if (in_array($uploadedFileExtension,$validFileExts)) {
                   if ( $dest !='' && JFile::upload($src, $dest) ) {                   		
						$query	= $db->getQuery(true);
						$query = "UPDATE #__yoorecipe SET published = 0 WHERE created_by=".(int)$userId." AND id=".(int)$recipeId;
						//print_r($sql);exit;
						$db->setQuery($query);
						$result = $db->query();
