 ...
 * @since       1.6
 */
class UsersModelPartner extends JModelForm
{
	/**
	...
	/**
	 * Method to get the Partner's recipe total social media track data.
	 *
	 * @param   int  The partner id.
	 * @return  array  	The socail media track data.
	 */
	public function getSocialStats($userId)
	{
            // This gonna kill us that it is too slow
            $cacheLife = 43200; // Define the cache life time.
            $cacheDir = JPATH_CACHE . "/leon/"; 
            $cacheFile = $cacheDir . "partner_social_states_" . $userId . ".cache";
            $result = getCache($cacheFile, $cacheLife);
		if($result === false){
                    $db = JFactory::getDBO();
                    $sql = "SELECT ua.type_id,COUNT(ua.recipe_id) AS sharecount FROM `#__user_activity` AS ua LEFT JOIN #__yoorecipe AS y ON y.id = ua.recipe_id  WHERE y.created_by = ".(int)$userId." GROUP BY type_id";
                    $db->setQuery($sql);
	...
	/**
	* Method to get average rating of Partner's recipe
	...
	*/
	...
	{
		$db = JFactory::getDBO();
		//$sql = "SELECT AVG(rating) FROM `#__recipe_rating` AS rr LEFT JOIN #__yoorecipe AS y ON y.id = rr.recipe_id  WHERE y.created_by = ".(int)$userId;
		$sql = "SELECT AVG(editor_rating) FROM `#__yoorecipe_editor_rating` AS rr LEFT JOIN #__yoorecipe AS y ON y.id = rr.recipe_id  WHERE y.created_by = ".(int)$userId;
		$db->setQuery($sql);
