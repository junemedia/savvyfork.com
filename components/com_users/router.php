...
function UsersBuildRoute(&$query)
{
	...
	if (empty($items))
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			// Check to see if we have found the registration menu item.
			if (empty($partnerregistration) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'partnerregistration'))
			{
				$partnerregistration = $items[$i]->id;
			}
			...
			// Check to see if we have found the partner menu item.
			if (empty($partner) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'partner'))
			{
				$partner = $items[$i]->id;
			}
			
			// Check to see if we have found the userdetail menu item.
			if (empty($userprofile) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'userdetail'))
			{
				$userprofile = $items[$i]->id;
			}
		}
		...
		} elseif ($registration)
		{
		 ...
		 elseif ($partnerregistration)
		{
			$default = $partnerregistration;
		} elseif ($login)
		{
			$default = $login;
		}
	...
	if (!empty($query['view']))
	{
		switch ($query['view'])
		{
			...
			case 'partnerregistration':
				if ($query['Itemid'] = $partnerregistration)
				{
					unset ($query['view']);
				} else {
					$query['Itemid'] = $default;
