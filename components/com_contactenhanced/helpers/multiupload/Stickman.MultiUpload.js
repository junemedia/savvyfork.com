var MultiUpload = new Class({
	...
	options: {
  	...
  	initialize: function(options){
	    ...
	    var input_element		= this.options.input_element;
	  	...
	  	this.parent				= input_element.getParent();
	  	//Get Max Caracter Limit box element
	  	this.limitReachedBox	= this.parent.getChildren('.mf-number-files');
		...
		if( this.remove_empty_element && !Browser.ie8){
			input_element.form.addEvent(
				'submit',function(){
					//get Form validator and only disable field if Form is valid;
					var validator	= new Form.Validator.Inline(this.options.formID);
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			current_element.element.style.left = '-99999px';
			new_input.inject( current_element.element, 'after' );
			
			if(this.elements.length > this.max ){
				this.limitReachedBox.set('html',this.options.language.txtnomorethan + ' ' + this.max + ' ' + this.options.language.txtfiles);
				this.limitReachedBox.addClass('file-limit-reached');
			}else{
				this.limitReachedBox.removeClass('file-limit-reached');
				this.limitReachedBox.set('html',this.options.language.filesleft + ' ' + (parseInt(this.max-this.elements.length)+1));
			}
		} else {
