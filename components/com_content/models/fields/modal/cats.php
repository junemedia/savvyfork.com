...
class JFormFieldModal_Cats extends JFormField
{
    ...
    #adminForm span.category { display: block; }
    ...
    $cjq(document).ready(function($) {
      $cjq().ready(function() {   
          ...
          link	= "index.php?option=com_content&view=categories&layout=modal&tmpl=component&catz="+id+"&function=jSelectCategory_'.$this->id.'";
  ...
  $cjq(document).ready(function($) {
    $cjq().ready(function() {  
      ...
      $cjq.ajax({
        ...
        data: rand+"&item="+item+"&title="+title+"&chck="+chck+"&client=site&rand2="+Math.random(),
...
';
		...
		$html	= array();
		$link	= JUri::root().'index.php?option=com_content&amp;view=categories&amp;layout=modal&amp;tmpl=component&amp;catz='.$this->value.'&amp;function=jSelectCategory_'.$this->id;
    ...
    $data = $mainframe->setUserState( "com_content.mcats", $catz );
		...
		$html[] = '	<a class="modal" title="'.JText::_('COM_CONTENT_CHANGE_CATEGORY').'"  href="'.$link.'&amp;'.JSession::getFormToken().'=1" rel="{handler: \'iframe\', size: {x: 800, y: 450}}">'.JText::_('COM_CONTENT_CHANGE_CATEGORY_BUTTON').'</a>';
		$html[] = '  </div>';
		$html[] = '</div>';
    ...
    //if($titles != '') {} 
    $html[] = '  <div style="position: relative; margin-top: 5px; -webkit-border-radius: 5px; border-radius: 5px; display: block; border: 1px solid #d5d5d5; padding: 5px 0 0 0px 10px 5px 10px; margin-bottom: 10px; " id="'.$this->id.'_title"><div id="catmask"></div>'.$titles.'</div>';
		$html[] = '</div>';
...
/*$html[] = '    <select id="jform_catid" name="jform[catid]" class="inputbox required">
	<option value="2" selected="selected">- Nezařazen&eacute;</option>
	<option value="32">- Demo obsah</option>
	<option value="33">- - Nějak&aacute; podkategorie </option>
	<option value="36">- Homepage</option>
	<option value="34">- Uk&aacute;zkov&eacute; čl&aacute;nky</option>
</select>';
*/                                                         
		return implode("\n", $html);
	}
