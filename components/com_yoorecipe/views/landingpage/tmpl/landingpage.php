...
<?php if ($canShowTitle) : ?>
<h1 class="yoorecipe-h1"><?php echo ($frontpageLbl == '') ? JText::_('COM_YOORECIPE_LANDING_PAGE') : $frontpageLbl; ?></h1>
<?php endif; ?>
...
<?php
	...
	foreach ($alphabet as $tmpLetter) :
		...
		if ($this->recipeStartLetters != null && in_array($tmpLetter , $this->recipeStartLetters)) {
			echo '<a title="' . $tmpLetter . '" href="' . JRoute::_('index.php?option=com_yoorecipe&task=getRecipesByLetter&l='. urlencode($tmpLetter)) . '">' . $tmpLetter . '</a>';
		} else {
...
if ($canBrowseByCategories && count($this->subcategories) > 0) {
	...
	echo '<div class="yoorecipe-hr">&nbsp;</div>';
}
