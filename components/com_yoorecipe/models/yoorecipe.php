...
class YooRecipeModelYooRecipe extends JModelList
{
	...
	public function getRecipeById($recipeId)
	{
        ...
        */
        ...
        $cacheFile = $cacheDir . "recipe_" . $recipeId . ".cache";
        //echo $queryMd5 . "<br>";
        //echo $queryString . "<br>";
        ...
        if($recipe === false){
		    ...
		    if (isset($recipe->published) && isset($recipe->validated)) {
			    ...
			    $recipe->rating = $this->getEditorRating($recipeId);
		    }
		    
		    //return $recipe;
            // Save the Cache
            saveCache($cacheFile,$recipe);
        }
        return $recipe;
	}
	...
	*/
	...
	/**
	* Get editor ratings
	* @return The editor ratings of the recipe
	*/
	public function getEditorRating($recipeId)
	{
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		
		$sql = 'SELECT editor_rating FROM #__yoorecipe_editor_rating WHERE recipe_id = '.$db->quote($recipeId);
	...
	*/
	...
	protected function getListQuery($categoryId = null, $orderBy = null)
	{
		...
		// Select some fields
		$query->select( 'SQL_CALC_FOUND_ROWS r.id, rr.editor_rating as rating,r.access, r.title, r.alias, r.description, r.created_by, r.preparation, r.servings_type' .
				', r.nb_persons, r.difficulty, r.cost, r.carbs, r.fat, r.saturated_fat, r.proteins, r.fibers, r.salt' .
				', r.kcal, r.kjoule, r.diet, r.veggie, r.gluten_free, r.lactose_free, r.creation_date, r.publish_up' .
        ...
        $query->join('LEFT', "#__yoorecipe_editor_rating rr on r.id=rr.recipe_id ");
	...
	*/
	...
	function getBigCardRecipes()
	{
		//Select sponsored partner, and exclude them
		$db = JFactory::getDBO();
		$sql = "SELECT recipe_id FROM #__yoorecipe_categories WHERE cat_id = 75"; //In stg site, the cat_id is 74
		$db->setQuery ($sql);
	...
	function getSponsoredRecipes($ids = array())
	{
        ...
        */
        ...
        $cacheFile = $cacheDir . "sponsered_" . md5(implode(',',$ids)) . ".cache";
		...
		{		
			// Select some fields
			$query->select( 'SQL_CALC_FOUND_ROWS r.id, r.access, r.title,rr.editor_rating as rating, r.alias, r.description, r.created_by, r.preparation, r.servings_type' .
					', r.nb_persons, r.difficulty, r.cost, r.carbs, r.fat, r.saturated_fat, r.proteins, r.fibers, r.salt' .
					', r.kcal, r.kjoule, r.diet, r.veggie, r.gluten_free, r.lactose_free, r.creation_date, r.publish_up' .
					', r.publish_down, r.preparation_time, r.cook_time, r.wait_time, r.picture, r.video, r.published' .
					', r.validated, r.featured, r.nb_views, r.note, r.metadata, r.metakey' .
	...
	function getRecipesByCatId($categoryId)
	{
        ...
        */
        $cacheLife = 3600; // Define the cache life time.
        $cacheDir = JPATH_CACHE . "/leon/";
        $cacheFile = $cacheDir . "sponsered_" . md5($categoryId) . ".cache";
        //echo $query;exit;
        $result = getCache($cacheFile, $cacheLife); 
