 ...
 * @since       11.1
 */
class JFormFieldYooCategory extends JFormFieldList
{
	/**
	 * The form field type.
	 *
	...
	protected function getOptions()
	{
		...
		require_once JPATH_COMPONENT_ADMINISTRATOR.'/helpers/html/yoocategory.php';
		$options = array();
		...
		if (!empty($extension))
		{
			...
			if ($published)
			{
				$options = JHtmlYooCategory::options($extension, array('filter.published' => explode(',', $published)));
			}
			else
			{
				$options = JHtmlYooCategory::options($extension);
			}
