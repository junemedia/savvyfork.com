...
class YooRecipeModelEditorrating extends JModelList
{
	...
	*/
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		// List state information
		$this->setState('orderCol', 'editor_rating');
	...
	protected function getListQuery()
	{
		...
		// Join over Cross Editor Rating
		$query->join('LEFT', '#__yoorecipe_editor_rating er on er.recipe_id = r.id');
		...
		if((int)$catId != 0)
		{
			$query->where('cc.cat_id ='.(int)$catId);
		}
		...
		$query->order('er.' . $this->getState('orderCol') . ' ' . $this->getState('drn'));
		$query->group('r.id');
