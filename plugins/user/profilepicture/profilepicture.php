...
class plgUserProfilePicture extends JPlugin
{
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		if( $files['error']['profilepicture']['file'] == 0 )
		{
			...
			$result = $this->profileImgUpload($userId,$files,2097152, PROFILEPICTURE_PATH_ORIGINAL);
			if($result)
			{
				$savedNewProfilePicture = true;
			}
	...
	private function profileImgUpload($userId,$file,$max, $filedir,$msg){

        if(isset($file)){
                //Clean up filename to get rid of strange characters like spaces etc
                $filename = JFile::makeSafe($file['name']['profilepicture']['file']);

                if($file['size']['profilepicture']['file'] > $max) $msg = JText::_('ONLY_FILES_UNDER').' '.$max;
                //Set up the source and destination of the file
				$uploadedFileNameParts = explode('.',$filename);
				$uploadedFileExtension = array_pop($uploadedFileNameParts);
                $src = $file['tmp_name']['profilepicture']['file'];
                $filename = sha1($userId.uniqid()).'.'.plgUserProfilePicture::FILE_EXTENSION;//sha1($userId.uniqid()).'.'.$uploadedFileExtension;
                ...
                if (in_array($uploadedFileExtension,$validFileExts)) {
                   if ( JFile::upload($src, $dest) ) {
                   		...
                   		$db = JFactory::getDBO ();
						$query	= $db->getQuery(true);
						$query = "INSERT INTO #__user_profiles (user_id,profile_key,profile_value) value($userId,".$db->quote(plgUserProfilePicture::PROFILE_KEY).",'".$filename."') ON DUPLICATE KEY UPDATE profile_value='".$filename."'";
						//print_r($sql);exit;
						$db->setQuery($query);
						$result = $db->query();
