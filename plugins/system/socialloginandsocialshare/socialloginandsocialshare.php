     ...
     // Check lr id exist.
      ...
      if (empty($check_id)) {
		...
		if ($db->execute()) {
		  ...
		  // Add new id to db.
		  $sql = "INSERT INTO #__LoginRadius_users SET id = " . JFactory::getUser()->id . ", LoginRadius_id = " . $db->Quote ($lrdata['id']).", provider = " . $db->Quote ($lrdata['Provider']) . ", lr_picture = " . $db->Quote ($userImage). ", account = " . $db->Quote ($lrdata['ProfileName']). ", email = " . $db->Quote ($lrdata['email']);
          $db->setQuery ($sql);
	      $db->execute();
        }
	 ...
	 $user_id = $db->loadResult();      
	  ...
	  if (empty($user_id)) {
		...
		if (!empty($user_id)) {
	      ...
	      if (empty($check_id) && $lr_settings ['linkaccount'] == '1') {
		    ...
		    // Add new id to db.
		    $userImage = plgSystemSocialLoginTools::add_newid_image($lrdata);
		    $sql = "INSERT INTO #__LoginRadius_users SET id = " . $user_id . ", LoginRadius_id = " . $db->Quote ($lrdata['id']).", provider = " . $db->Quote ($lrdata['Provider']) . ", lr_picture = " . $db->Quote ($userImage). ", account = " . $db->Quote ($lrdata['ProfileName']). ", email = " . $db->Quote ($lrdata['email']);
            $db->setQuery ($sql);
	        $db->execute();
		  }
		}
	  ...
	  // Trying to insert image.
        ...
        if ($db->execute()) {
		  ...
		  //Add new id to db
          $sql = "INSERT INTO #__LoginRadius_users SET id = " . $db->quote ($user_id) . ",  LoginRadius_id = " . $db->Quote ($lrdata['id']).", provider = " . $db->Quote ($lrdata['Provider']).", lr_picture = " . $db->Quote ($userImage). ", account = " . $db->Quote ($lrdata['ProfileName']). ", email = " . $db->Quote ($lrdata['email']);
          $db->setQuery ($sql);
          $db->execute();
	    }         
