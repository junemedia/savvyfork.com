<?php
/*------------------------------------------------------------------------
# plg_community_yoorecipe_yourcookbook - YooRecipe! Joomla 1.6 recipe component
# ------------------------------------------------------------------------
# author    YooRock!
...
class plgCommunityYoorecipe_YourCookbook extends CApplications{
	...
	/** Constructor */
	function plgCommunityYoorecipe_YourCookbook (&$subject, $config) {
	...
	function onProfileDisplay(){
		...
		JPlugin::loadLanguage( 'plg_community_yoorecipe_yourcookbook', JPATH_SITE );
		...
		// Load css
		$document = JFactory::getDocument();
		$document->addStyleSheet(JUri::base() . 'plugins/community/yoorecipe_yourcookbook/yoorecipe_yourcookbook/style.css');
	
		ob_start();
		
		$appfiles_path 	= 'plugins/community/yoorecipe_yourcookbook/yoorecipe_yourcookbook/';
		$my_id 			= $this->_my->id;
	...
	function _getRecipesQuery($user_id) {
		...
		$query->where('r.created_by = ' . $db->quote($user_id));
	...
	function _displayRecipes($recipes, $user_id) {
		...
		foreach($recipes as $recipe) {
			...
			$html[] = '<div id="div-recipe-' . $recipe->id .'" class="yoorecipe-row row-' . $cnt++ . '">';
			...
			$html[] = '<a href="' . $url . '" title="'. htmlspecialchars($recipe->title) . '" target="self">' . htmlspecialchars($recipe->title) . '</a>';
			$html[] = '</div>';
			
			$html[] = '<div class="fav-icon" id="fav_' . $recipe->id . '">';
			if (!$user->guest && $yooRecipeparams->get('use_favourites', 1) == 1 ) :
				if ($recipe->favourite) {
					$html[] = '<a onclick="yrckbk_removeFromFavourites('.$recipe->id.')"><img src="media/com_yoorecipe/images/favourites.png" alt="'.JText::_('COM_YOORECIPE_REMOVE_FROM_FAVOURITES').'" title="'.JText::_('COM_YOORECIPE_REMOVE_FROM_FAVOURITES').'"/></a>';
				} else {
					$html[] = '<a onclick="yrckbk_addToFavourites('.$recipe->id.')"><img src="media/com_yoorecipe/images/add-to-favourites.png" alt="'.JText::_('COM_YOORECIPE_ADD_TO_FAVOURITES').'" title="'.JText::_('COM_YOORECIPE_ADD_TO_FAVOURITES').'"/></a>';
				}
			endif;
			$html[] = '</div>';
			...
			$html[] = '<span id="span-recipe-' . $recipe->id . '" class="yoorecipe-loading"></span>';
			if ($recipe->canEdit) {
			...
			if ($recipe->canDelete) {
				$html[] = '<button type="button" class="btn" id="btn_delete_' . $recipe->id . '">' . JText::_('COM_YOORECIPE_DELETE') . '</button>';
			}
			$html[] = '</div>';
			$html[] = '</div>';
			$html[] = '</div>';
	...
	public function _getDeleteAJAXScript($recipe_id) {
		...
		$url = JURI::root().'index.php?option=com_yoorecipe&task=deleteRecipe&format=raw&id='.$recipe_id;
		return "window.addEvent('domready', function () {
			
	$('btn_delete_".$recipe_id."').addEvent('click', function () {

	if (confirm('".addslashes(JText::_('PLG_COMMUNITY_YOORECIPE_CONFIRM'))."')) {
	
		$('span-recipe-".$recipe_id."').setStyle('display', 'block');
		var a = new Request({
			url: '".$url."',
			method:'post',
			onSuccess:function(response){
				...
				if (response.match('^'+'NOK')) {
					...
					$('span-recipe-".$recipe_id."').setStyle('display', 'none');
				} else {
					$('div-recipe-".$recipe_id."').set('tween', {
						duration: 600
					}).fade('out').get('tween').chain(function() {
						$('div-recipe-".$recipe_id."').tween('height',0);
					}).chain(function () {
						$('div-recipe-".$recipe_id."').dispose();
					});
				}
			},
			onFailure: function(){
	...
	public function getAddToFavouritesScript()
	{
		...
		return "//<![CDATA[
		function yrckbk_addToFavourites(recipeId) {
			...
			var x = new Request({
				...
				onSuccess: function(result){
					...
					var a = new Element('a');
					a.addEvent('click', function() { yrckbk_removeFromFavourites(recipeId); }); 
	...
	public function removeFromFavouritesScript()
	{
		...
		return "//<![CDATA[
		function yrckbk_removeFromFavourites(recipeId) {
			...
			var x = new Request({
				...
				onSuccess: function(result){
					...
					$('fav_'+recipeId).removeClass('ajax-loading');
					var a = new Element('a');
					a.addEvent('click', function() {yrckbk_addToFavourites(recipeId); }); 
