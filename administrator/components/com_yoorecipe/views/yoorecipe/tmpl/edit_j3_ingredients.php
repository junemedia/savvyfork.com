...
if ($this->ingredients) { 
	...
	foreach ($this->ingredients as $crtIngredient) {
		...
		$label = $qtyValue.' '.addslashes(JText::_($crtIngredient->unit)).' '.addslashes(trim($crtIngredient->description));
		...
		var inputDescriptionElt = new Element('input', {'type':'hidden', 'size':'3', 'name':'ingr_description[]', 'value':'".addslashes(trim($crtIngredient->description))."'});
