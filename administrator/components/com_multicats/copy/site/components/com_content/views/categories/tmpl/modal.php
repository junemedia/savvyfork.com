...
function checkCategory(id, item, title, chck) {
      ...
      jQuery.ajax({
        ...
        data: "item="+item+"&title="+title+"&chck="+chck+"&catz="+catzz+"&catztitles="+catztitles+"&client=site",
...
#catmask {width: 100%; height: 100%; position: absolute; display: none; background: rgba(255,255,255,0.7) url('.JURI::root().'administrator/components/com_multicats/assets/images/loading.gif) center 0px no-repeat; }
');
?>

<form class="catform" action="<?php echo JRoute::_('index.php?option=com_content&view=categories&layout=modal&tmpl=component&function='.$function.'&'.JSession::getFormToken().'=1');?>" method="post" name="adminForm" id="adminForm">
<?php 
...
?>
  ...
  <input type="inputbox" id="catztitles" name="catztitles" value="<?php echo $catztitles; ?>" size="60" readonly="readonly"/>
...
?>
      ...
      $originalOrders = array();
        ...
        $canChange = false;
            ...
            ?>
            <input class="cats" <?php echo $chck;?> type="checkbox" id="cb<?php echo $i; ?>" name="cid[]" value="<?php echo $item->id; ?>" onclick="checkCategory(id, <?php echo $item->id;?>, <?php echo '&quot;'.$this->escape($item->title).'&quot;'; ?>, this.checked);" title="" />
					</td>
					<td>
...
$('<div>', {id:"catmask" } ).prependTo("form.catform");  //recreate catmask
</script>