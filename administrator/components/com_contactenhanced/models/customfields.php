...
class ContactenhancedModelCustomfields extends JModelList
{
	...
	protected function getListQuery($resolveFKs = true)
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'cf.id, cf.name, cf.required, cf.value, cf.type, cf.tooltip, cf.published, cf.access, cf.ordering, cf.catid, cf.language')
		);
		...
		elseif (is_array($categoryId)) {
			...
			$query->where('cf.catid IN ('.$categoryId.')');
		}
		...
		if ($orderCol == 'cf.ordering' || $orderCol == 'c.title' || $orderCol == 'category_title') {
			$orderCol = 'c.title '.$orderDirn.', cf.ordering';
		}
