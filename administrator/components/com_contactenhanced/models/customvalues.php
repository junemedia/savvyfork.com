...
class ContactenhancedModelCustomvalues extends JModelList
{
	 ...
	 * @since	1.6
	 */
	protected function populateState($ordering = 'cv.value', $direction = 'asc')
	{
	...
	protected function getListQuery($resolveFKs = true)
	{
		...
		$query->join('LEFT', '#__categories AS c ON c.id = cv.catid');
		...
		$orderCol	= $this->state->get('list.ordering', 'cv.ordering');
		$orderDirn	= $this->state->get('list.direction', 'ASC');
		if ($orderCol == 'cv.ordering' || $orderCol == 'cv.category' || $orderCol == 'cv.catid') {
			$orderCol = 'cv.catid '.$orderDirn.', cv.ordering';
		}
