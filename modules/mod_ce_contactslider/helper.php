...
class modCEContactSliderHelper {
	/**
	 * get instance of modCEContactSliderHelper
	 */
	function getInstance(){
		static $__instance = null;
		if( !$__instance ){
			$__instance = new modCEContactSliderHelper();
		}
		return $__instance;
	}
	...
	public static function getList(&$params)
	{
		...
		$moduleMode = $params->get('module_mode', 'normal');
		...
		switch ($moduleMode)
		{
			case 'dynamic':
				...
				if ($option === 'com_contactenhanced') {
					switch($view)
					{
						...
						case 'categories':
							...
							break;
						case 'contact':
							if ($params->get('module_mode-dynamic-show_on_contact_page', 1)) {
				...
				elseif($params->get('module_mode-dynamic-show_on_contactenhanced_pages_only', 0)) {
					// Return right away if not on a com_contactenhanced page
					return array();
				}else{
					//Mimics normal module mode
		...
		$misc_limit			= $params->get('misc_limit', 40);
		...
		foreach ($items as &$item)
		{
			...
			$item->href		= modCEContactSliderHelper::contactHREF($item, $params);

			if($item->image){
				$item->image = modCEContactSliderHelper::renderImage( $item->title
																	, $item->href
																	, $item->image
																	, $params
	...
	public static function renderImage( $title, $link, $image, $params, $width = 0, $height = 0, $attrs='', $returnURL=false ) {
		...
		$image = '<a href="'.$link.'" title="'.$title.'" class="contact-image ce-image">'.$image.'</a>';
		// clean up globals
		return $image;
	}
	...
	public static function javascript( $params  ){
		...
		$document =JFactory::getDocument();
		$document->addScript( JURI::base().'modules/mod_ce_contactslider/assets/js/ce_contactslider.js' );
	}
	/**
	...
	public static function css( $params ){
		...
		$cssFile  = 'ce_contactslider.css';

		if( file_exists(JPATH_SITE.'/templates/'.$mainframe->getTemplate().'/css/'.$cssFile) ) {
			$document->addStyleSheet( JURI::base().'templates/'.$mainframe->getTemplate().'/css/'.$cssFile );
		} else {
			$document->addStyleSheet( JURI::base().'modules/mod_ce_contactslider/assets/css/ce_contactslider.css' );
		}
	}
	/**
